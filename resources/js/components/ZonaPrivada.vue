    ...
    <div>
        ...
        <div style="margin-bottom: 350px;">
                        ...
                        <div class="d-flex flex-column w-100 d-none d-lg-block">
                            <div class="d-flex justify-content-between">
                                    ...
                                    <div class="d-flex nav-item" @click="abrirEmail()" style="cursor: pointer;">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                            ...
                                            <path
                                                ...
                                                stroke="#E3202B" stroke-width="2" stroke-linecap="round"
                                ...
                                <div>
                                    <div>
                                        <router-link to="/home"
                                            :style="{ fontWeight: isRouteActive('/home') ? 'bold' : '700' }">
                                            <img class="logoNavbar" :src="getImagen(this.imagenLogo)" alt="">
                                        </router-link>
                                    </div>
                                </div>
                                ...
                                <div style="margin-top: 32px;">
                                    <div>
                                        <button type="button" class="btn Bzona">{{ this.nombre }}</button>
                                    </div>
                                </div>
                ...
                <div class="container-fluid">
                    ...
                    <div class="container collapse navbar-collapse" id="navbarSupportedContent"
                        ...
                        <div class="d-flex flex-column w-100">
                            <div id="datos" class="d-flex justify-content-end aling-items-center w-100"
                                style="background: #E3202B;">

                                <div>
                                    <router-link class="route">
                                        <p class="textoNavbar2"
                                            :style="{ fontWeight: isRouteActive('/empresa') ? '700' : '500' }"></p>
                                    </router-link>
                                </div>
                                <div>
                                    <router-link class="route">
                                        <p class="textoNavbar2"
                                            :style="{ fontWeight: isRouteActive('/productos') ? '700' : '500' }"></p>
                                    </router-link>
                                </div>
                                <div>
                                    <router-link class="route">
                                        <p class="textoNavbar2"
                                            :style="{ fontWeight: isRouteActive('/servicios') ? '700' : '500' }"></p>
                                    </router-link>
                                </div>
                                <div>
                                    <router-link class="route">
                                        <p class="textoNavbar2"
                                            :style="{ fontWeight: isRouteActive('/descargas') ? '700' : '500' }"></p>
                                    </router-link>
                                </div>
                                <div>
                                    <router-link class="route">
                                        <p class="textoNavbar2"
                                            :style="{ fontWeight: isRouteActive('/novedades') ? '700' : '500' }"></p>
                                    </router-link>
                                </div>
                                <div>
                                    <router-link class="route">
                                        <p class="textoNavbar2"
                                            :style="{ fontWeight: isRouteActive('/ferias') ? '700' : '500' }"></p>
                                    </router-link>
                                </div>
                                <div>
                                    <router-link class="route">
                                        <p class="textoNavbar2"
                                            :style="{ fontWeight: isRouteActive('/videos') ? '700' : '500' }"></p>
                                    </router-link>
                                </div>
                                <div>
                                    <p style="margin-right: 20px;" class="textoNavbar2">Lista de precios</p>
                                </div>
                                <div><a target="_blank" :href="this.instagram">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="23" height="24"
                                            viewBox="0 0 23 24" fill="none" style="margin-bottom: 6px;">
                                            <path
            ...
            <div class="container">
                ...
                <div class="table-responsive" style="padding-top: 52px">
                    <table class="table">
                        <thead>
                            <tr>
                                <th style="padding: 30px; padding-left: 0px;" scope="col" class="encabezado">N° de
                                    pedido
                                </th>
                                <th style="padding: 30px; padding-left: 0px;" scope="col" class="encabezado">Formato
                                </th>
                                <th style="padding: 30px; padding-left: 0px;" scope="col" class="encabezado">Peso</th>
                                <th style="padding: 30px;" scope="col" class="encabezado col-lg-1"></th>

                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="descarga in descargas" :key="descarga.id" style="padding: 20px;">
                                <th class="ths">{{ descarga.numeropedido }}</th>
                                <th class="ths">{{ descarga.formato.toUpperCase() }}</th>
                                <th class="ths">{{ descarga.peso.substring(0, 4) }}Kb</th>
                                <th>
                                    <div class="d-flex justify-content-end align-items-center"
                                        style="gap: 20px; margin-top: 20px">
                                        <a :href="'/api/obtenerZona/' + descarga.id" target="_blank" style="text-decoration: none;">
                                            <button id="verO" type="button" class="btn">Ver online</button>
                                        </a>
                                        <button id="Des" type="button" class="btn"
                                            @click="descargarArchivo(descarga.id, descarga.numeropedido)">Descargar</button>
                                    </div>
...
export default {
    ...
    methods: {
        ...
        descargarArchivo(id, nameArchive) {
            axios.get(`/api/descargarArchiveZona/${id}`, { responseType: 'blob' })
                .then(response => {
        ...
        obtenerUsuario() {
            axios.get(`/api/obtenerIdUsuarioZona/${this.getIdLogin}`)
                .then(response => {
        ...
        obtenerContacto() {
            axios.get(`/api/obtenerContacto`)
                .then(response => {
                    ...
                    this.whatsappLink = response.data[0].telefono
